...
class FreightFrenzyAllianceDetails {
  ...
  static FreightFrenzyAllianceDetails fromMap(map) {
    return FreightFrenzyAllianceDetails(
      ...
      navigated1: _navLocationFromString(map['auto_navigated_1']),
      ...
      carouselPoints: map['carousel_points'],
...
NavLocation _navLocationFromString(String str) {
  return NavLocation.values.firstWhere((element) => element.name == str);
}

String navLocationToString(NavLocation location, TOALocalizations local) {
  return local.get('breakdowns.freightfrenzy.' + location.name.toLowerCase());
}

enum BarcodeElement { DUCK, TEAM_SHIPPING_ELEMENT }

BarcodeElement _barcodeElementfromString(String str) {
  return BarcodeElement.values.firstWhere((element) => element.name == str);
}

String barcodeElementToString(BarcodeElement element, TOALocalizations local) {
  return local.get('breakdowns.freightfrenzy.' + element.name.toLowerCase());
}
